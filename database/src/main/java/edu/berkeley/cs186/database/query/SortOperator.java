...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Run mergedRun = new Run(transaction, getSchema());
        List<Iterator<Record>> list = new ArrayList<>();
        for (int i = 0; i < runs.size(); i++) {
            Run run = runs.get(i);
            list.add(run.iterator());
            queue.add(new Pair<>(list.get(i).next(), i));
        }
        while (!queue.isEmpty()) {
            Pair<Record, Integer> p = queue.poll();
            mergedRun.add(p.getFirst());
            int idx = p.getSecond();
            if (list.get(idx).hasNext()) {
                queue.add(new Pair<>(list.get(idx).next(), idx));
            }
        }
        return mergedRun;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            Iterator iterator = getBlockIterator(sourceIterator, getSchema(), numBuffers);
            sortedRuns.add(sortRun(iterator));
        }
