...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (Integer i : getEligibleIndexColumns(table)) {
            SelectPredicate curr = this.selectPredicates.get(i);
            QueryOperator indexOp = new IndexScanOperator(transaction, table, curr.column, curr.operator, curr.value);
            int cost = indexOp.estimateIOCost();
            if (cost < minCost) {
                minCost = cost;
                minOp = indexOp;
                chooseIdx = i;
        ...
        minOp = addEligibleSelections(minOp, chooseIdx);
        return minOp;
    }
    ...
    private QueryOperator minCostJoinType(QueryOperator leftOp,
                                          ...
                                          String leftColumn,
                                          String rightColumn) {
        QueryOperator minCostOp = null;
        int minCost = Integer.MAX_VALUE;
        JoinOperator SNLJOp = new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
        JoinOperator BNLJOp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Map<Set<String>, QueryOperator> resultMap = new HashMap<>();
        for (Map.Entry<Set<String>, QueryOperator> entry : prevMap.entrySet()) {
            for (JoinPredicate joinOperator : this.joinPredicates) {
                Set<String> tables = entry.getKey();
                Set<String> tableSet = new HashSet<>();
                Set<String> newSet = new HashSet<>(tables);
                QueryOperator minOp = null;
                if (!tables.contains(joinOperator.leftTable) && tables.contains(joinOperator.rightTable)) {
                    tableSet.add(joinOperator.leftTable);
                    QueryOperator leftOp = pass1Map.get(tableSet);
                    minOp = minCostJoinType(entry.getValue(), leftOp, joinOperator.rightColumn, joinOperator.leftColumn);
                    newSet.add(joinOperator.leftTable);
                } else if (tables.contains(joinOperator.leftTable) && !tables.contains(joinOperator.rightTable)) {
                    tableSet.add(joinOperator.rightTable);
                    QueryOperator rightOp = pass1Map.get(tableSet);
                    minOp = minCostJoinType(entry.getValue(), rightOp, joinOperator.leftColumn, joinOperator.rightColumn);
                    newSet.add(joinOperator.rightTable);
                } else {
                    continue;
                }
                if (resultMap.containsKey(newSet)) {
                    QueryOperator preOp = resultMap.get(newSet);
                    if (preOp.estimateIOCost() > minOp.estimateIOCost()) {
                        resultMap.put(newSet, minOp);
                    }
                } else {
                    resultMap.put(newSet, minOp);
                }
    ...
    private QueryOperator minCostOperator(Map<Set<String>, QueryOperator> map) {
        ...
        for (Map.Entry<Set<String>, QueryOperator> entry : map.entrySet()) {
            int cost = entry.getValue().estimateIOCost();
            if (cost < minCost) {
                minCost = cost;
    ...
    public Iterator<Record> execute() {
        ...
        for (String table : tableNames) {
            ...
            tableSet.add(table);
            prevMap.put(tableSet, minOp);
            pass1Map.put(tableSet, minOp);
        }
