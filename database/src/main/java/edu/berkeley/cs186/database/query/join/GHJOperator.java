...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        // to get a hash value.
        int columnIdx = left ? getLeftColumnIndex() : getRightColumnIndex();
        for (Record record : records) {
            int partitionNum = HashFunc.hashDataBox(record.getValue(columnIdx), pass) % partitions.length;
            if (partitionNum < 0) {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record buildRecord : buildRecords) {
            if (!hashTable.containsKey(buildRecord.getValue(buildColumnIndex))) {
                hashTable.put(buildRecord.getValue(buildColumnIndex), new ArrayList<>());
            }
            List<Record> buildRecordList = hashTable.get(buildRecord.getValue(buildColumnIndex));
            buildRecordList.add(buildRecord);
            hashTable.put(buildRecord.getValue(buildColumnIndex), buildRecordList);
        }

        for (Record probeRecord : probeRecords) {
            if (hashTable.containsKey(probeRecord.getValue(probeColumnIndex))) {
                for (Record buildRecord : hashTable.get(probeRecord.getValue(probeColumnIndex))) {
                    joinedRecords.add((probeFirst ? probeRecord.concat(buildRecord) : buildRecord.concat(probeRecord)));
                }
            }
        }
    }
