...
public class LockContext {
    ...
    public void acquire(TransactionContext transaction, LockType lockType)
        ...
        if (parentContext != null) {
            parentContext.updateChildLockNum(transaction.getTransNum(), 1);
        }
    }

    private void updateChildLockNum(long tranNum, int delta) {
        numChildLocks.put(tranNum, numChildLocks.getOrDefault(tranNum, 0) + delta);
        LockContext parentContext = parentContext();
        if (parentContext != null) {
            parentContext.updateChildLockNum(tranNum, delta);
        }
    }

    private void updateChildLockNum(long tranNum, int delta, List<ResourceName> names) {
        for (ResourceName resourceName : names) {
            LockContext lockContext = fromResourceName(lockman, resourceName);
            if (lockContext != null) {
                lockContext.updateChildLockNum(tranNum, delta);
            }
        }
    }
    ...
    public void promote(TransactionContext transaction, LockType newLockType)
        ...
        if (newLockType == LockType.SIX) {
            ...
            updateChildLockNum(transaction.getTransNum(), -1, resourceNames);
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        boolean toX = (curType == LockType.IX || curType == LockType.SIX || curType == LockType.X);

        if (!toX) {
            for (ResourceName descendant : descendants) {
                LockType lockType = lockman.getLockType(transaction, descendant);
        ...
        if (descendants.isEmpty() && !curType.isIntent()) {
            return;
        }
    ...
    private boolean hasSIXAncestor(TransactionContext transaction) {
        ...
        while (parentContext != null) {
            ResourceName resourceName = parentContext.getResourceName();
            LockType lockType = lockman.getLockType(transaction, resourceName);
    ...
    private List<ResourceName> sisDescendants(TransactionContext transaction) {
        // TODO(proj4_part2): implement
        List<ResourceName> retDescendants = new ArrayList<>();
        for (ResourceName resourceName : getDescendants(transaction)) {
            if (lockman.getLockType(transaction, resourceName) == LockType.IS || lockman.getLockType(transaction, resourceName) == LockType.S) {
                retDescendants.add(resourceName);
            }
        }
