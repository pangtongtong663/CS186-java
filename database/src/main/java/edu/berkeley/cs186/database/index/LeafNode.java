...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() <= 2 * order) {
            ...
            List<RecordId> newRids = rids.subList(order, keys.size());
            ...
            sync();
            return Optional.of(new Pair<DataBox, Long>(newKeys.get(0), newLeaf.getPage().getPageNum()));
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (data.hasNext()) {
            ...
            ret =  Optional.of(new Pair<>(p.getFirst(), newNode.page.getPageNum()));
        }
        sync();
        return ret;
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        byte type = buf.get();
        assert (type == 1);
        Long rightSiblingVal = buf.getLong();
        Optional<Long> rightSibling = Optional.empty();
        if (rightSiblingVal != -1L) {
            rightSibling = Optional.of(rightSiblingVal);
        }
        int len = buf.getInt();
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();
        ...
        return new LeafNode(metadata, bufferManager, page, newKeys, newRids, rightSibling, treeContext);
    }
