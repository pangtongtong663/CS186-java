...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> bulkInfo = rightMostNode.bulkLoad(data, fillFactor);
        if (!bulkInfo.isPresent()) {
            return bulkInfo;
        } else {
            Pair<DataBox, Long> p = bulkInfo.get();
            Optional<Pair<DataBox, Long>> splitInfo = insert(p.getFirst(), p.getSecond());
            if (splitInfo.isPresent()) {
                return splitInfo;
            } else {
