...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        LogRecord commitRecord = new CommitTransactionLogRecord(transNum, transactionTableEntry.lastLSN);
        logManager.appendToLog(commitRecord);
        logManager.flushToLSN(commitRecord.LSN);
        transaction.setStatus(Transaction.Status.COMMITTING);
        transactionTableEntry.lastLSN = commitRecord.LSN;
        return commitRecord.LSN;
    }
    ...
    public long abort(long transNum) {
        ...
        transactionTableEntry.lastLSN = abortRecord.LSN;
        return abortRecord.LSN;
    }
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        LogRecord writeRecord = new UpdatePageLogRecord(transNum, pageNum, prevLSN, pageOffset, before, after);
        long LSN = logManager.appendToLog(writeRecord);
    ...
    void restartAnalysis() {
        ...
        while (iter.hasNext()) {
            ...
            if (record.getTransNum().isPresent()) {
                ...
                Iterator<Map.Entry<Long, Long>> DPTIterator = record.getDirtyPageTable().entrySet().iterator();
                Iterator<Map.Entry<Long, Pair<Transaction.Status, Long>>> TXNIterator = record.getTransactionTable().entrySet().iterator();
                while (DPTIterator.hasNext()) {
                    Map.Entry<Long, Long> dirtyPage = DPTIterator.next();
                    if (!dirtyPageTable.containsKey(dirtyPage.getKey()) || dirtyPageTable.get(dirtyPage.getKey()) > dirtyPage.getValue()) {
                        dirtyPageTable.put(dirtyPage.getKey(), dirtyPage.getValue());
                    }
                }
                while (TXNIterator.hasNext()) {
                    Map.Entry<Long, Pair<Transaction.Status, Long>> Txn = TXNIterator.next();
                    ...
                    if (!transactionTable.containsKey(transNum)) {
                        Transaction transaction = newTransaction.apply(transNum);
                        if (Txn.getValue().getFirst() == Transaction.Status.ABORTING) {
                            transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
                        } else {
                            transaction.setStatus(Txn.getValue().getFirst());
                        }
                        startTransaction(transaction);
                        transactionTable.get(transNum).lastLSN = Txn.getValue().getSecond();
                    } else {
                        Transaction.Status newStatus = Txn.getValue().getFirst();
                        long newLSN = Txn.getValue().getSecond();
                        TransactionTableEntry transactionTableEntry = transactionTable.get(transNum);
                        Transaction transaction = transactionTableEntry.transaction;
                        if (transactionTableEntry.lastLSN < newLSN) {
                            transactionTableEntry.lastLSN = newLSN;
                        }
                        if (isAdvanced(newStatus, transaction.getStatus())) {
        ...
        while (TXNIterator.hasNext()) {
            Map.Entry<Long, TransactionTableEntry> Txn = TXNIterator.next();
            long transNum = Txn.getKey();
            TransactionTableEntry transactionTableEntry = Txn.getValue();
            Transaction transaction = transactionTableEntry.transaction;
    ...
    private boolean isAdvanced(Transaction.Status a, Transaction.Status b) {
        switch (a) {
            case COMMITTING:
            case ABORTING: return (b == RUNNING);
        ...
        while (iter.hasNext()) {
            ...
            if (record.isRedoable()) {
                ...
                if (logType == LogType.UPDATE_PAGE || logType == LogType.UNDO_UPDATE_PAGE || logType == LogType.FREE_PAGE || logType == LogType.UNDO_ALLOC_PAGE) {
                    ...
                    try {
                        if (!dirtyPageTable.containsKey(pageNum) || dirtyPageTable.get(pageNum) > record.LSN || page.getPageLSN() >= record.LSN) {
                            flag = false;
                        }
                    } finally {
                        page.unpin();
    ...
    void restartUndo() {
        // TODO(proj5): implement
        PriorityQueue<Long> queue = new PriorityQueue<>((Long v1, Long v2) -> v2.compareTo(v1));
        for (TransactionTableEntry transactionTableEntry : transactionTable.values()) {
            queue.offer(transactionTableEntry.lastLSN);
        }
        while (!queue.isEmpty()) {
            long maxLSN = queue.poll();
            LogRecord record = logManager.fetchLogRecord(maxLSN);
            long transNum = record.getTransNum().get();
            TransactionTableEntry transactionTableEntry = transactionTable.get(transNum);
